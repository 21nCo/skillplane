...
-- Workspace data remains fenced while migration outboxes are allowed to
-- acknowledge work admitted before the placement entered the moving state.
...
BEGIN
  ...
  -- Enqueueing is still fenced. Only acknowledgements and retry bookkeeping
  -- may change after quiescence, so the final retained snapshot cannot omit a
  -- newly admitted workspace event.
  IF TG_TABLE_NAME IN (
