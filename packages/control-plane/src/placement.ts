...
export interface WorkspaceRegionCandidate {
...
export interface WorkspaceClientLocation {
  readonly latitude?: number;
  readonly longitude?: number;
...
/**
 * Selects the physically nearest declared cell from trusted edge coordinates.
 * A continent center is used only when the edge does not provide coordinates.
 */
export function selectNearestWorkspaceRegion(
  location: WorkspaceClientLocation | null,
  candidates: readonly WorkspaceRegionCandidate[],
): string | null {
  if (!location) return null;
  const source: readonly [number, number] | undefined =
    finiteCoordinate(location.latitude, -90, 90) &&
    finiteCoordinate(location.longitude, -180, 180)
      ? [location.latitude, location.longitude]
      : location.continent
        ? CONTINENT_CENTERS[location.continent.toUpperCase()]
        : undefined;
  ...
  const located = candidates
        ...
        ? [
            {
              ...
              distance: distanceKilometers(source, [latitude, longitude]),
            },
          ]
        : [];
    })
    .sort(
      (left, right) =>
        left.distance - right.distance || left.regionId.localeCompare(right.regionId),
    );
  return located[0]?.regionId ?? null;
}

/** Stable fallback used only when trusted edge location is unavailable. */
export function selectInitialWorkspaceRegion(
  ...
  if (preferredRegionId && regions.includes(preferredRegionId)) {
