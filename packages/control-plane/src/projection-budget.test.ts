...
it("finishes in-flight work past the soft deadline and releases unused chunk claims", async () => {
  let clock = 0;
  const time = vi.spyOn(performance, "now").mockImplementation(() => clock);
  ...
  try {
    ...
    expect(statements[2]).toContain("WHERE claim_token = $1 AND processed_at IS NULL");
  } finally {
    time.mockRestore();
  }
});
